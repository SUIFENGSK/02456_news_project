...
class NewsEncoder(nn.Module):
    def __init__(self, hparams, word2vec_embedding, input_dimensions, use_positional_encoding, debug=False):
        ...
        if self.use_positional_encoding:
            self.positional_encoder = PositionEncoder(
                input_dimensions, hparams.title_size, use_learned_positions=hparams.use_learned_positions)

        self.self_attention = SelfAttention(
            input_dimensions, hparams.head_num, hparams.head_dim)
    ...
    def forward(self, sequences_input_title):
        ...
        if self.debug:
            print("NE0: Shape of input:", sequences_input_title.shape,
                  ". Should be (batch_size, title_size)")
        sequences_input_title = sequences_input_title.long()

        if self.debug:
            print("NE2: Shape after casting to long:",
                  sequences_input_title.shape, ". Should be (batch_size, title_size)")
        ...
        if self.debug:
            print("NE3: Shape after embedding:", embedded_sequences.shape,
                  ". Should be (batch_size, title_size, embedding_dim)")
        ...
        if self.debug:
            print("NE7: Shape after att layer:", y.shape,
                  ". Should be (batch_size, head_num * head_dim)")
    ...
    def forward(self, news_representation, user_representation):
        if self.debug:
            print("CP1: Shape of news_representation:", news_representation.shape,
                  ". Should be (candidate_size, batch_size, head_num * head_dim)")
        ...
        if self.debug:
            print("CP3: Reshape of news_representation:", news_representation.shape,
                  ". Should be (batch_size, candidate_size, head_num * head_dim)")
        ...
        if self.debug:
            print("CP3: Shape of prob:", prob.shape,
                  ". Should be (batch_size, candidate_size)")
        return prob
...
class NRMSModel(nn.Module):
    def __init__(self, hparams, word2vec_embedding, debug=False):
        ...
        self.news_encoder = NewsEncoder(hparams, tensor_word2vec_embedding,
                                        word2vec_embedding.shape[1], hparams.use_positional_encoding, debug)
        self.user_encoder = UserEncoder(
            hparams, self.news_encoder, debug)
    ...
    def forward(self, pred_input_title, his_input_title):
        if self.debug:
            print("Model: Shape of pred_input_title:", pred_input_title.shape,
                  ". Should be (batch_size, candidate_size, title_size)")
            print("Model: Shape of his_input_title:", his_input_title.shape,
                  ". Should be (batch_size, history_size, title_size)")
        user_representation = self.user_encoder(
            his_input_title)  # u in the paper
        news_representations = torch.stack([self.news_encoder(
            title) for title in pred_input_title], dim=1)  # r in the paper
        if self.debug:
