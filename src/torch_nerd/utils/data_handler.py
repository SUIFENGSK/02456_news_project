...
class NewsDataset:
    ...
    def setup_test_data(self, dataset_path: Path, datasplit: str, history_size: int, columns: list[str], fraction: float, seed: int):
        self.df_test = (beh.ebnerd_from_path(dataset_path.joinpath(datasplit, "validation"), history_size=history_size, padding=0,).select(columns).pipe(beh.sampling_strategy_wu2019, npratio=4, shuffle=True, with_replacement=True, seed=seed).pipe(beh.create_binary_labels_column).sample(fraction=fraction)
)
